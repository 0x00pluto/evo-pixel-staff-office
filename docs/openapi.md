...
1. `GET /api/catalog` → 取 `agents[].id`（必须是花名册里已有的 id）。
2. `POST /api/presence` 报到：
   - 开干 → `working`
   - 卡住 **或** 做完待验收 → `blocked`（**禁止**刚做完直接 `idle`）
   - 灭灯 / 已读 → `idle`（与大屏点黄灯等价）
3. `GET /api/presence` 自检全员有效出勤。

可选鉴权：设了 `EVO_PRESENCE_TOKEN` 时，跨机 POST 需 `Authorization: Bearer <token>`；同源浏览器可免。未设则开放。

最短 curl 示例见 [`README.md`](../README.md)「实时出勤」节。

## 怎么改合同

1. 改实现（[`presence-store.mjs`](../src/cli/presence-store.mjs) / [`presence-http.mjs`](../src/cli/presence-http.mjs) / catalog 插件）以及 [`src/cli/openapi.mjs`](../src/cli/openapi.mjs)。
...
相关：PRD [`specs/prds/prd-00007-live-presence.md`](../specs/prds/prd-00007-live-presence.md)；开发指南 [`docs/dev-guide.md`](./dev-guide.md)。
